...
# The single review model. Kimi K3 (via OpenRouter) — chosen after the
# multi-model consensus basket proved unreliable in practice (Gemini's endpoint
# erroring and DeepSeek returning empty responses), which made the "consensus"
# collapse to one working model anyway. One dependable model is simpler and more
# honest than a basket that silently degrades. Override with `--model`.
...
def build_parser() -> argparse.ArgumentParser:
    ...
    review.add_argument(
        ...
        default=None,
        help=(
            f"Model string (any Pydantic AI model string) for the review. "
    ...
    review.add_argument(
        ...
        help=(
            ...
            f"(at high/blocker severity). The literal 'default' expands "
            f"to {DEFAULT_MODEL}. Off by default."
        ),
    )
    review.add_argument(
        "--skip-precheck",
        ...
        help=(
            ...
            "review invocation probes the resolved model (reviewer + optional "
            "verifier) to confirm it supports structured tool-call output "
            "before dispatching the real review. Pass this flag if the probe "
            "itself is flaky or you want to deliberately attempt a "
            "known-borderline model."
        ),
    )
    review.add_argument(
        "--no-date-guard",
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "review":
        ...
        try:
            ...
            return 2
        review_model = args.model or DEFAULT_MODEL
        # Precheck (Layer 0) — runs unless --skip-precheck.
        if not args.skip_precheck:
            models_to_check = [review_model]
        ...
        return asyncio.run(run_review_command(
            ...
            out=args.out, model=review_model, exclude=args.exclude,
            verifier_model=verifier_model,
